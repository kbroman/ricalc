...
README file for the ricalc package
----------------------------------------------------------------------
This explains the installation of the ricalc package for R.  If you
...
OBTAINING R/ricalc
  ...
  You can obtain the latest version of R/ricalc from 

      http://www.biostat.wisc.edu/~kbroman/software


INSTALLATION OF R AND R/ricalc (Windows)
  ...
  2. To install R/ricalc, you may do one of the following:

     Start R.  Select (on the menu bar) "Packages" and then
     "Install package from local zip file...".  Find the file
     "ricalc.zip" on your hard drive, and click "Open".


INSTALLATION OF R/ricalc (Unix)

  1. We'll assume that R has already been installed. 

  2. Go into the directory containing the file "ricalc_*.tar.gz".

  3. Do one of the following:

     a. To install R/ricalc in the standard location
        (/usr/local/lib/R/library), type 

            R INSTALL ricalc_*.tar.gz

        You'll probably need to be superuser.

     b. To install the package locally, type 

            R INSTALL --library=/home/auser/Rlibs ricalc_*.tar.gz
...
GETTING STARTED
  ...
  Once you start R, you'll need to type "library(ricalc)" to load
  the package.  You can create a file "~/.Rprofile" (Unix),
  "c:\.Rprofile" (Windows)containing R code to be run whenever you
  start R.  If you use the R/ricalc package regularly, you should
  place the line "library(ricalc)" in such a file.

  To get started with R/ricalc, you might first peruse the
...
QUESTIONS/COMMENTS/CONCERNS
  ...
  regarding R/ricalc, please email Karl Broman <kbroman@biostat.wisc.edu>.
